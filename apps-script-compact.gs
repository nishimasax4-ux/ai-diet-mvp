/**
 * Training Log + AI  Apps Script (v41.2) 貼り付け用・短縮版
 *
 * コメントを取り除いただけで、動作は配布物の apps-script.gs と完全に同じです
 * (同じ自動テスト20件に通しています)。スマホでの貼り付けが途中で切れにくいよう、
 * 行数を 600行 → 425行 に減らしてあります。
 *
 * 【貼り付け手順】
 *  1. Apps Scriptエディタで、今あるコードを「全選択 → 削除」して空にする
 *  2. このファイルの中身を全部貼り付ける
 *  3. 保存する
 *  4. 一番下までスクロールし、最後の行が「}」で終わっていることを確認する
 *  5. デプロイ → デプロイを管理 → 鉛筆マーク → バージョン「新バージョン」→ デプロイ
 *
 * 【スクリプトプロパティ】
