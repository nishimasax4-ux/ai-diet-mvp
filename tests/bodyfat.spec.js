...
test.describe('training-log v41.2 — 古いApps Scriptの検知とレイアウト', () => {
  test('古いバージョンのApps Scriptを検知して、再デプロイを促す', async ({ page }) => {
    await page.route('https://script.google.com/macros/**', (route, request) => {
      const body = JSON.parse(request.postData() || '{}');
      // v39相当: mealPlanは知っているが古い(ai の状態も返さない)
      const out = body.action === 'ping'
        ? { status: 'ok', backendVersion: 'v39' }
    ...
    expect(await page.evaluate(() => backendOutdatedByVersion_())).toBe(true);
    const html = await page.locator('#tab-content').innerHTML();
    expect(html).toContain('Apps Scriptが古いバージョンです');
    expect(html).toContain('45秒待っても応答がありませんでした');
  });

  test('最新のApps Scriptなら警告は出ない', async ({ page }) => {
    await page.route('https://script.google.com/macros/**', (route, request) => {
      const body = JSON.parse(request.postData() || '{}');
      const out = body.action === 'ping'
    ...
    expect(await page.evaluate(() => backendOutdatedByVersion_())).toBe(false);
    expect(await page.locator('#tab-content').innerHTML()).not.toContain('古いバージョンです');
  });

  test('AIが時間切れになったら、原因(古い版数)を突き止めて文言に足す', async ({ page }) => {
    await page.route('https://script.google.com/macros/**', (route, request) => {
      const body = JSON.parse(request.postData() || '{}');
      if (body.action === 'ping') {
        route.fulfill({ status: 200, contentType: 'application/json',
          body: JSON.stringify({ status: 'ok', backendVersion: 'v39' }) });
        return;
      }
      if (body.action === 'mealPlan') return;   // 応答を返さない = 時間切れ
      route.fulfill({ status: 200, contentType: 'application/json',
        body: JSON.stringify({ status: 'ok', payload: {}, lastWriteAt: null, written: 0 }) });
    });
    await boot(page);
    await seed(page, [{ date: today(), weight: 78, bodyFat: 29 }]);
    const msg = await page.evaluate(async () => {
      gasConfig = { url: 'https://script.google.com/macros/s/fake/exec', token: 't', lastKnownWriteAt: null };
      try { await gasPost({ action: 'mealPlan' }, 1200); return 'NO_ERROR'; }
      catch (e) { return await diagnoseAiFailure_(e.message); }
    });
    expect(msg).toContain('応答がありませんでした');
    expect(msg).toContain('古いバージョン(v39)');
    expect(msg).toContain('新バージョン');
  });

  test('体重フォームの日付欄は単独の行に置かれ、他の欄と重ならない', async ({ page }) => {
    for (const width of [320, 375, 390, 430]) {
      ...
      const m = await page.evaluate(() => {
        ...
        const f = document.getElementById('weight-bodyfat-input').getBoundingClientRect();
        const ov = (a, b) => Math.abs(a.top - b.top) < 4 && a.right > b.left + 0.5 && a.left < b.right - 0.5;
        return {
          dateAlone: Math.abs(d.top - v.top) >= 4 && Math.abs(d.top - f.top) >= 4,
          anyOverlap: ov(d, v) || ov(d, f) || ov(v, f),
        };
      });
      expect(m.dateAlone, `幅${width}px`).toBe(true);     // 日付は必ず単独の行
      expect(m.anyOverlap, `幅${width}px`).toBe(false);
    }
  });
});
